...
def sendSensorDataLocal(sensor_data):
    ...
    while True:
        ...
        if ackpacket is None:
            ...
            pkt_id = prev_packet #get the ID of the packet sent
            #check that the packet id matches this one
            print("Recieved Packet from ack",pkt_id,"\n")
            print("Actual ID",id_byte,"\n")

            if pkt_id == id_byte:
                display.text('success!', 15, 20, 1)
                print("Successfully Received Ack from Service Station")
                display.show()
                time.sleep(1)
                return
